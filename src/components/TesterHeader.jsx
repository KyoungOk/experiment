...
export default function TesterHeader() {
	...
	const { getTester } = TesterListViewModel();
	...
	const handleTesterClick = (tester) => {
		...
		navigate("/profileTester"); // tester가 아니라면 리디렉션
	};
	const handleLogoClick = (tester) => {
		// 원하는 동작 실행: 상세 보기, 모달 열기, 페이지 이동 등
		navigate("/homeTester"); // tester가 아니라면 리디렉션
	};
	return (
		<>
			<div className="sticky top-0 z-50 bg-white border-b border-gray-100">
				<div className="flex items-center justify-between px-[20px] md:px-[var(--side-padding)] pt-5 pb-4 md:pt-8 md:pb-5 text-black">
					...
					<div
						...
						<div className="text-valueTitle">{tester?.name || ""}</div>
					</div>
				</div>
			</div>
		</>
